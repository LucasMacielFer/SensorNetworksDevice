...
class MPL3115A2:
    ...
    def pressure(self):
        ...
        # 1. Faz o cálculo dos bits
        raw = (OUT_P_MSB[0] << 10) | (OUT_P_CSB[0] << 2) | ((OUT_P_LSB[0] >> 6) & 0x03)
        
        # 2. Adiciona a parte decimal
        decimal = ((OUT_P_LSB[0] >> 4) & 0x03) / 4.0
        
        final_pressure = float(raw + decimal)

        # 3. CORREÇÃO DO BUG: Se o valor for absurdamente baixo (< 80000 Pa), soma o bit perdido
        if final_pressure < 80000:
            final_pressure += 65536.0
